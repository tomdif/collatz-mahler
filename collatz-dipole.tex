...
\title[Spectral Rigidity of the Collatz Koopman Operator]{The Dipole Conjecture: Spectral Rigidity of the Collatz Koopman Operator in the Mahler Basis}
...
a striking phenomenon: the kernel of the operator $(I - \Koopman_N)$ is consistently 
2-dimensional, spanned by the constant function and an alternating vector that 
approximates $\delta_0 - \delta_{-1}$.

We prove that $\dim \ker(I - M_N) \geq 1$ for all $N$, with the lower bound achieved 
by the exact eigenvector $\delta_0$ corresponding to the fixed point at $0$. 
...
We also analyze the invariant measure on the 2-cycle $\{1, 2\}$, explaining why it 
does not produce a third kernel vector in finite truncations, and establish that 
the accelerated Collatz map acts as an isometry on the 2-adic units, explaining 
why standard expanding-map ergodic theorems do not apply.
\end{abstract}
...
approximating $\delta_0 - \delta_{-1}$, so $\dim \ker(I - M_N) = 2$.
...
with small corrections near position $N$.
\end{itemize}

\item \textbf{The 2-Cycle Obstruction} (Section~\ref{sec:algebraic}): 
The invariant measure $(\delta_1 + \delta_2)/2$ on the 2-cycle $\{1, 2\}$ is 
an eigenvector of the full Koopman operator but does not produce a third exact 
kernel vector in finite truncations. We analyze why this occurs.

\item \textbf{Isometry Obstacle} (Section~\ref{sec:isometry}): The map $T$ 
preserves the 2-adic norm on units: $|T(x)|_2 = |x|_2 = 1$ for odd $x$. This 
explains why standard ergodic theorems for expanding maps do not apply.
\end{enumerate}
...
spectral properties.
\end{itemize}
We \emph{do} prove the lower bound $\dim \ker(I - M_N) \geq 1$ (Theorem~\ref{thm:dipole-lower}), 
and we provide strong computational evidence that the dimension is exactly 2.
...
$M_N$ with entries
\[
(M_N)_{m,n} = \sum_{k=0}^{n} (-1)^{m-k} \binom{m}{k} \binom{T(k)}{n}, \quad 0 \leq m, n < N.
\]
...
performed with exact rational arithmetic.}
\label{tab:nullity}
\end{table}
...
\end{align*}
The vector $v_2$ agrees with the alternating pattern $(-1)^{n+1}$ for positions 
$1 \leq n \lesssim N/2$ and has small corrections thereafter.
\end{observation}
...
\begin{proof}
For $x \geq 0$, we have $\binom{x}{n} = 0$ for $n > x$, so
\[
f_2(x) = \sum_{n=1}^{x} (-1)^{n+1} \binom{x}{n} = -\sum_{n=1}^{x} (-1)^{n} \binom{x}{n} = -\left[\sum_{n=0}^{x} (-1)^n \binom{x}{n} - 1\right] = -(0 - 1) = 1
\]
for $x \geq 1$ (using the binomial theorem), and $f_2(0) = 0$ since all terms 
with $n \geq 1$ vanish.
\end{proof}
...
and $\delta_{-1}$ is a distribution, not a function. The identity is purely 
algebraic at the level of coefficient sequences.
\end{remark}
...
The accelerated Collatz map $T$ preserves the 2-adic absolute value on odd 
integers: for all odd $x \in \Z$,
\[
|T(x)|_2 = |x|_2 = 1.
\]
\end{theorem}
...
\begin{corollary}
The map $T$ is not uniformly expanding on the 2-adic units. Standard theorems 
for expanding maps (such as those guaranteeing unique ergodicity) do not apply.
\end{corollary}
...
pattern $[0, 1, -1, 1, -1, \ldots]$. Consequently, $\dim \ker(I - M_N) = 2$ 
for all $N \leq 500$.
\end{observation}
...
for all $N$, which we have not established.
\end{remark}
...
\midrule
20 & $\approx 10^{8}$ & 6 & 3 \\
50 & $\approx 10^{24}$ & 12 & 3 \\
100 & $\approx 10^{50}$ & 20 & 3 \\
\bottomrule
\end{tabular}
\caption{Spectral characteristics of the truncated Koopman matrices $M_N$. 
The maximum eigenvalue magnitude grows exponentially with $N$, demonstrating 
...
\begin{enumerate}
\item \textbf{Stable modes} ($|\lambda| \approx 1$): The table shows 3 eigenvalues 
near 1 for all tested $N$. Two of these correspond to the kernel (eigenvalue exactly 1 
with geometric multiplicity 2). The third eigenvalue is very close to but not exactly 1 
(typically $|\lambda - 1| \approx 10^{-6}$ to $10^{-10}$); it appears to arise from the 
near-invariance of the 2-cycle $\{1, 2\}$ in the truncated system. As $N$ increases, 
this third eigenvalue moves closer to 1 but does not produce a third kernel vector.
...
indicator of nonzero integers (corresponding to the basin of the cycle 
$\{1, 2, 4\}$).
...
there is a 5-cycle $\{1, 3, 8, 4, 2\}$ (under appropriate conventions).

We computed $\dim \ker(I - M_N^k)$ for the $5x+1$ map at truncation $N = 30$ for 
periods $k = 1, 2, \ldots, 10$. The results show a dramatic \emph{resonance} at 
period 5:

\begin{center}
\begin{tabular}{@{}cc@{}}
\toprule
Period $k$ & $\dim \ker(I - M_N^k)$ \\
\midrule
1 & 2 \\
2 & 2 \\
3 & 2 \\
...
The nullity spike from 2 to 6 at period 5 corresponds exactly to the 5-cycle 
contributing 4 additional approximate eigenvectors (the non-trivial roots of unity 
modes). This confirms that the spectral method successfully detects cycles when 
they are present.

For the Collatz map, no such resonances appear: $\dim \ker(I - M_N^k) = 2$ for all 
odd $k$, and $\dim \ker(I - M_N^k) = 3$ for all even $k$ (reflecting only the 
known 2-cycle $\{1, 2\}$). The complete absence of unexpected nullity spikes 
provides strong computational evidence against the existence of non-trivial 
cycles in the Collatz dynamics.
...
corresponding to the fixed points at $0$ and $-1$. This ``dipole structure'' 
appears to be a fundamental feature of the Collatz map in the Mahler basis.
The positive control experiment on the $5x+1$ map---where the method successfully 
detects a known 5-cycle via a nullity spike---validates the approach. The complete 
absence of analogous spectral resonances in the Collatz case provides compelling 
computational evidence against the existence of non-trivial cycles.
...
We hope these results---a rigorous partial result and a precisely stated conjecture 
